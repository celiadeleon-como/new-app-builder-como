export function initPromoWidgets(ctx) {
  ...
  function createImageBanner() {
    ...
    file.addEventListener('change', async () => {
      const data = await readImageFile(file.files?.[0]);
      if (!data) return;
      preview.src = data;
      preview.style.display = '';
      empty.style.display = 'none';
      el.querySelector('.hb-img').style.backgroundImage = 'url(' + data + ')';
      el.querySelector('.hb-img').classList.remove('is-empty');
      markDirty();
    });
    ...
    drillNode.querySelector('.hb-btn').addEventListener('input', (e) => {
      const btn = el.querySelector('.hb-cta');
      btn.textContent = e.target.value;
      btn.hidden = !e.target.value.trim();
      markDirty();
    });
